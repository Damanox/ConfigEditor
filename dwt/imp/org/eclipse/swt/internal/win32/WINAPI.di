// D import file generated from 'org\eclipse\swt\internal\win32\WINAPI.d'
...
version (Tango)
{
	...
	extern (Windows) WINBOOL CreateProcessAsUserA(HANDLE, LPCSTR, LPSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES, WINBOOL, DWORD, LPVOID, LPCSTR, LPSTARTUPINFO, LPPROCESS_INFORMATION);
...
extern (Windows) 
{
	...
	HPAINTBUFFER BeginBufferedPaint(HDC hdcTarget, RECT* prcTarget, uint dwFormat, void* pPaintParams, HDC* phdc);
	...
	LRESULT CoCreateInstance(byte* rclsid, void* pUnkOuter, int dwClsContext, byte* riid, void* ppv);
}
extern (Windows) 
{
	...
	BOOL GradientFill(HDC hdc, PTRIVERTEX pVertex, DWORD dwNumVertex, PVOID pMesh, DWORD dwNumMesh, DWORD dwMode);
	...
	HRESULT SHGetMalloc(LPVOID* ppMalloc);
	version (WinCE)
	{
	...
	HRESULT ScriptCPtoX(UTF16index iCP, BOOL fTrailing, UTF16shift cChars, int cGlyphs, WORD* pwLogClust, SCRIPT_VISATTR* psva, int* piAdvance, SCRIPT_ANALYSIS* psa, int* piX);
	...
	HRESULT ScriptGetLogicalWidths(SCRIPT_ANALYSIS* psa, UTF16shift cChars, int cGlyphs, int* piGlyphWidth, WORD* pwLogClust, SCRIPT_VISATTR* psva, int* piDx);
	HRESULT ScriptGetProperties(SCRIPT_PROPERTIES*** ppSp, int* piNumScripts);
	HRESULT ScriptGetCMap(HDC hdc, SCRIPT_CACHE* psc, LPCWSTR pwcInChars, int cChars, DWORD dwFlags, WORD* pwOutGlyphs);
	HRESULT ScriptStringAnalyse(HDC hdc, LPCVOID pString, int cString, int cGlyphs, int iCharset, DWORD dwFlags, int iReqWidth, SCRIPT_CONTROL* psControl, SCRIPT_STATE* psState, int* piDx, SCRIPT_TABDEF* pTabdef, BYTE* pbInClass, SCRIPT_STRING_ANALYSIS* pssa);
	HRESULT ScriptStringOut(SCRIPT_STRING_ANALYSIS ssa, int iX, int iY, UINT uOptions, RECT* prc, UTF16index iMinSel, UTF16index iMaxSel, BOOL fDisabled);
	...
	HRESULT ScriptShape(HDC hdc, SCRIPT_CACHE* psc, LPCWSTR pwcChars, int cChars, int cMaxGlyphs, SCRIPT_ANALYSIS* psa, WORD* pwOutGlyphs, WORD* pwLogClust, SCRIPT_VISATTR* psva, int* pcGlyphs);
	HRESULT ScriptTextOut(HDC hdc, SCRIPT_CACHE* psc, int x, int y, UINT fuOptions, RECT* lprc, SCRIPT_ANALYSIS* psa, WCHAR* pwcReserved, int iReserved, WORD* pwGlyphs, int cGlyphs, int* piAdvance, int* piJustify, GOFFSET* pGoffset);
	HRESULT ScriptXtoCP(int iX, UTF16shift cChars, int cGlyphs, WORD* pwLogClust, SCRIPT_VISATTR* psva, int* piAdvance, SCRIPT_ANALYSIS* psa, UTF16shift* piCP, UTF16shift* piTrailing);
...
extern (C) int wcslen(wchar* string);
